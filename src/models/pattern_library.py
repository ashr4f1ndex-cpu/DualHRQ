...
class PatternLibrary:
    ...
    def find_similar_patterns(self, query_features: Dict[str, Any], top_k: int = 10) -> List[Pattern]:
        ...
        with self._lock:
            ...
            # Create query vector from features
            ...
            query_vector = query_pattern.to_feature_vector().reshape(1, -1)
            query_vector = self._scaler.transform(query_vector)
            ...
            if len(candidate_indices) <= top_k:
                ...
                # Optimized similarity computation using cosine similarity
                ...
                normalized_query = query_vector_flat / query_norm
                similarities = np.dot(normalized_candidates, normalized_query)
                
                # Get top-k indices (highest similarity) using argpartition for speed
                if len(similarities) <= top_k:
                    top_k_indices = np.argsort(similarities)[::-1]  # All, sorted descending
                else:
    ...
    def _rebuild_similarity_index(self):
        ...
        if not self._patterns:
            self._feature_vectors = np.array([]).reshape(0, 13)  # 13 features in feature vector
            self._pattern_ids_ordered = []
            return
        
        # Pre-allocate arrays for efficiency
        n_patterns = len(self._patterns)
        feature_vectors = np.empty((n_patterns, 13), dtype=np.float32)
        pattern_ids = []
        
        # Extract feature vectors efficiently
        for i, (pid, pattern) in enumerate(self._patterns.items()):
            feature_vectors[i] = pattern.to_feature_vector()
        ...
        if len(feature_vectors) > 0:
            # Use partial fit for large datasets if available
            if hasattr(self._scaler, 'partial_fit'):
                self._scaler.partial_fit(self._feature_vectors)
            else:
